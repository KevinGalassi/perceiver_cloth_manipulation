...
def compute_surface_curvature(point_cloud, k_neighbors=20):
   ...
   curvatures     = torch.zeros((n_points, 2), dtype=torch.float32)
   curvature_norm = torch.zeros(n_points, dtype=torch.float32)
   normals        = torch.zeros((n_points, 3), dtype=torch.float32)
...
def compute_fpfh_1(point_cloud, radius, num_neighbors):
   ...
   fpfh_features = np.zeros((point_cloud.shape[0], num_neighbors), dtype=np.float32)
   ...
   def compute_fpfh_for_point(query_point, neighbors, radius):
      ...
      unique_counts = np.unique(neighbors, axis=0, return_counts=True)[1]

      # Normalize the counts by the volume of the shell defined by the radius
      fpfh_feature = unique_counts / (np.pi * radius ** 2 * distances.sum())
