...
class HeatPerceiver(nn.Module):
   ...
   def __init__(self, **kwargs) -> None:
      ...
      # Action parameters
      action_embedding_dim = kwargs.get('action_embedding_dim', 128)
      ...
      self.action_predciton = ActionPredictor(
                        input_embedding_dim = action_embedding_dim,
   ...
   def forward(self, pts):
      ...
      output = self.action_predciton.forward(pt, pts)
   ...
   def trainer_action(self, train_loader, device='cuda') :
      ...
      #for i, data in tqdm(enumerate(train_loader), total=len(train_loader), desc='Training Progress'):
         ...
         pt_taken = pts[torch.arange(b), torch.where(gt_gaussian == 1)[1]].unsqueeze(1)

         a = self.action_predciton.forward(pt_taken, pts)
         a = a.reshape(b, -1)
...
class ActionPredictor(nn.Module):
   ...
   def __init__(self, **kwargs) -> None:
      ...
      self.pt_embedding = nn.Sequential(
         nn.Linear(3, input_embedding_dim//2),
         nn.ReLU(),          
         nn.Linear(input_embedding_dim//2, input_embedding_dim),
         nn.ReLU(),
   ...
   def forward(self, pt_taken, pts) :
      ...
      ptse = self.pts_embedding(pt_taken)
      pte    = self.pt_embedding(pts)

      x = self.cross_attention(ptse, context = pte)
      x = self.cross_ff(x) + x
